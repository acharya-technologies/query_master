...
if (isset($_GET['saveInTxt'])) {
    $sql = "SELECT *,(submitTime-loginTime) as time FROM $tbl_usr ORDER BY id";
    $result = $con->query($sql);
    $file = "./users-" . $_GET['gpin'] . ".txt";
    $data = "Sr. No.\t\tUID\t\tName\t\t\t\tMarks\t\tTime Taken\n\n";
    $data2 = '';
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $data2 .= $row['id'] . "\t\t" . $row['username'] . "\t\t" . $row['name'] . "\t\t" . $row['marks'] . "/20\t\t" . number_format($row['time'], 2) . "s\n";
        }
    }
    file_put_contents($file, $data);
    file_put_contents($file, $data2, FILE_APPEND);
    echo "<script>//location.href = './users" . $gpin . ".txt'</script>";
...
<body>
    ...
    <?php if (!isset($_GET['action'])) { ?>
        ...
        <form action="" class="flex items-center w-1/2 m-auto my-10">
            <input type="submit" name="action"
                class="px-4 py-2 w-30 text-blue-500 m-auto rounded hover:bg-blue-500 hover:text-white border-2 border-blue-500"
        ...
        <form id="cards" class="grid md:grid-cols-3 grid-cols-1" action="" method="get">
            ...
            if ($records > 0) {
                ...
                while ($rowGamepin = mysqli_fetch_assoc($result)) { ?>
                    <div class="w-full Gotu max-w-sm mx-auto my-4 bg-gray-100 border border-gray-200 rounded-lg shadow">
                        <img class="p-2 rounded-t-lg" src="http://source.unsplash.com/1600x900/?<?php for ($j = 1; $j <= $i; $j++)
                            echo "gradient,"; ?>" alt="<?php echo $rowGamepin['title']; ?>" />
                        <div class="px-5 pb-5">
                            <h5 class="text-xl  text-gray-900 ">
                                <?php echo $rowGamepin['title']; ?>
                            </h5>
                            <div class="flex mt-4 items-center justify-between ">
                                <input type="button" onclick="preview<?php echo $i; ?>()" name="action"
                                    class="px-4 py-2 text-green-500 rounded hover:bg-green-500 hover:text-white border-2 border-green-500"
                                    value="Preview" />
                                <input type="button" onclick="manage<?php echo $i; ?>()" name="action"
                                    class="px-4 py-2 text-blue-600 rounded hover:bg-blue-600 hover:text-white border-2 border-blue-600"
                                    value="Manage" />
            ...
            <div class="w-full Gotu max-w-sm mx-auto my-4 bg-gray-100 border border-gray-200 rounded-lg shadow">
                <svg part="svg" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" aria-labelledby="icon-add"
                    focusable="false" viewBox="0 0 24 24" class="h-icon icon-primary aspect-video h-icon--no-custom-width">
                    <!---->
                    <g>
                ...
                <div class="px-5 pb-5">
                    ...
                    <div class="flex mt-8 items-center justify-center">
                        <input type="button" onclick="numQuestion()" name="action"
                            class="px-4 py-2 text-purple-500 rounded hover:bg-violet-500 hover:text-white border-2 border-purple-500"
                            value="New">
                    </div>
                </div>
            </div>
    ...
    if (isset($_GET['action'])) {
            ...
            <form autocomplete="off" class="m-auto Poppins h-full bg-slate-100 w-full flex flex-col justify-center items-center"
                ...
                if ($records > 0) {
                    while ($rowGamepin = mysqli_fetch_assoc($result)) { ?>
                        ...
                        <label>Quiz Pin
                            <input value="<?php echo $gpin; ?>" id="gamepin" type="number" name="gamepin"
                                class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                readonly />
                            <i class="fa fa-refresh ml-3 text-xl motion-safe:hover:animate-spin text-gray-500"
                                onclick="generatePin()"></i>
                        </label><br>
                        <label class="-ml-10 pb-4 border-b-2 border-gray-500"> Quiz Title
                            <input id="gameTitle" name="gametitle"
                                class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                value="<?php echo $rowGamepin['title']; ?>" onkeyup="replaceme(gameTitle)" required />
                        </label><br>
                        <hr>
                ...
                for ($i = 1; $i <= 20; $i++) {
                    ...
                    if ($records > 0) {
                        while ($rowQuestion = mysqli_fetch_assoc($result)) {
                            ...
                            <input type="number" name="serialQuestion<?php echo $rowQuestion['qno']; ?>"
                                value="<?php echo $rowQuestion['srno']; ?>" id="serialQuestion<?php echo $rowQuestion['qno']; ?>"
                                class="w-10 hidden focus:outline-none focus:border-slate-600 border-2 border-slate-400 rounded text-center"
                                readonly />
                            <input id="quen<?php echo $rowQuestion['qno']; ?>" type="number" min="1" max="20"
                                name="quen<?php echo $rowQuestion['qno']; ?>" onchange="check<?php echo $rowQuestion['qno']; ?>()"
                                class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 rounded px-0 w-12 text-center pl-2"
                                value="<?php echo $rowQuestion['qno']; ?>" required /><br>
                            <textarea id="question<?php echo $rowQuestion['qno']; ?>" name="question<?php echo $rowQuestion['qno']; ?>"
                                class="focus:outline-none w-1/3 bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                onkeyup="replaceme(question<?php echo $rowQuestion['qno']; ?>)"
                                required><?php echo $rowQuestion['question']; ?></textarea> <br>
                            ...
                            <div class=" px-4 py-2">
                                ...
                                if ($records2 > 0) {
                                    while ($rowAnswer = mysqli_fetch_assoc($result2)) { ?>
                                        ...
                                        <label>
                                            <input type="number" name="serialAnswer<?php echo $rowQuestion['qno']; ?>"
                                                value="<?php echo $rowAnswer['srno']; ?>" id=" serialAnswer<?php echo $rowQuestion['qno']; ?>"
                                                class="w-10 hidden focus:outline-none focus:border-slate-600 border-2 border-slate-400 rounded text-center"
                                                readonly />
                                            <?php $x = ($rowAnswer['ano'] % 4);
                                            if ($x == 0)
                                                $x = 4;
                                            echo $x; ?>)
                                            <input id="option<?php echo $i . $t ?>" name="option<?php echo $rowQuestion['qno']; ?>"
                                                class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                                onkeyup='replaceme(option<?php echo $i . $t; ?>)' value="<?php echo $rowAnswer['answer']; ?>"
                                                required />
                                        </label><br><br>
                                    <?php }
                                } ?>
                            ...
                            <select id="select<?php echo $rowQuestion['qno']; ?>" name="select<?php echo $rowQuestion['qno']; ?>"
                                onchange="check<?php echo $rowQuestion['qno']; ?>()"
                            ...
                            <option value="1" <?php if ($rowQuestion['correct'] % 4 == 1)
                                echo "selected"; ?>>Option 1</option>
                            <option value="2" <?php if ($rowQuestion['correct'] % 4 == 2)
                                echo "selected"; ?>>Option 2</option>
                            <option value="3" <?php if ($rowQuestion['correct'] % 4 == 3)
                                echo "selected"; ?>>Option 3</option>
                            <option value="4" <?php if ($rowQuestion['correct'] % 4 == 0)
                                echo "selected"; ?>>Option 4</option>
                            </select><br>
                            <input type="number" name="correct<?php echo $rowQuestion['qno']; ?>"
                                id="correct<?php echo $rowQuestion['qno']; ?>"
                                class=" w-10 focus:outline-none focus:border-slate-600 border-2 border-slate-400 rounded text-center hidde"
                                readonly />
                            <br>
                            <script>
                                ...
                                function check<?php echo $rowQuestion['qno']; ?>() {
                                    correct<?php echo $rowQuestion['qno']; ?>.value = parseInt(select<?php echo $rowQuestion['qno']; ?>.value) +
                                        4 * (parseInt(quen<?php echo $rowQuestion['qno']; ?>.value) - 1);
                                }
                            </script>
                            </div>
                            </div><br>
                ...
                <div align="center">
                    <input type="submit" id="update" onclick="refresh();" name="update"
        ...
        ?>
            ...
            if ($_GET['action'] == "New") {
                ...
                <form autocomplete="off"
                    class="m-auto Poppins h-full w-full flex bg-gray-100 flex-col justify-center items-center"
                    autocomplete="off" action="" method="post"> <br><br>


                    <?php if (!isset($_POST['add']) && $_GET['create'] > 0) {
                        ?>
                        <label>Quiz Pin
                            <input value="<?php echo $rowGamepin['gpin']; ?>" id="gamepin" type="number" name="gamepin"
                                class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                readonly />
                            <i class="fa fa-refresh ml-3 text-xl motion-safe:hover:animate-spin text-gray-500"
                                onclick="generatePin()"></i>
                        ...
                        <label class="-ml-10 pb-4 border-b-2 border-gray-500"> Quiz Title
                            <input id="gameTitle" name="gametitle"
                                class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                onkeyup="replaceme(gameTitle);document.title=this.value;" required />
                        </label><br>
                        <hr>
                        <?php
                        ...
                        while ($i <= $_GET['create']) { ?>
                            ...
                            <input id="quen<?php echo $i; ?>" min="1" max="20" type="number" name="quen<?php echo $i; ?>"
                                onchange="check()"
                                class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                value="<?php echo $i; ?>" required /><br>
                            <textarea id="question<?php echo $i; ?>" name="question<?php echo $i; ?>"
                                class=" focus:outline-none bg-slate-100 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                placeholder="Enter Question" onkeyup='replaceme(question<?php echo $i; ?>)' required /></textarea><br>
                            ...
                            <label>2) <input id="option<?php echo $i; ?>B" name=" option<?php echo $i; ?>B"
                                    class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                    onkeyup='replaceme(option<?php echo $i; ?>B)' required />
                            </label><br>
                            <label>3) <input id="option<?php echo $i; ?>C" name=" option<?php echo $i; ?>C"
                                    class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                    onkeyup='replaceme(option<?php echo $i; ?>C)' required />
                            </label><br>
                            <label>4) <input id="option<?php echo $i; ?>D" name=" option<?php echo $i; ?>D"
                                    class="focus:outline-none bg-slate-200 border-2 border-slate-400 focus:border-slate-600 text-center rounded"
                                    onkeyup='replaceme(option<?php echo $i; ?>D)' required />
                            </label><br>
                        ...
                        <input type="submit" onclick="check()" name="add"
                            class="px-4 py-2 mx-auto btn btn-info border-2 border-slate-400 hover:text-white focus:bg-cyan-600 focus:text-white  text-slate-400 rounded focus:outline-none"
                            value="Insert Question" /><br><br>
                ...
                <?php }
                    if (isset($_POST['add'])) {
                        ...
                        ?>
                    <h1
                        class="m-auto w-screen h-screen flex flex-col justify-center items-center text-gray-500  text-center text-2xl Yatra">
                        Inserted
                        <?php echo $quen; ?> Questions.<br><a class="hover:underline" href="./">Please, get back and refresh page
                            before re-entering.</a>
                    </h1>
                <?php } ?>
                ...
                <script>
                    ...
                    function check() {
                        <?php for ($i = 1; $i <= $_GET['create']; $i++) { ?>
                            correct<?php echo $i; ?>.value = parseInt(select<?php echo $i; ?>.value) + 4 * (parseInt(
                                quen<?php echo $i; ?>.value) - 1);
                        <?php } ?>
                    }
            ...
            if ($_GET['action'] == "Manage") {
                ...
                <br>
                <h1 class="text-xl mt-10 mx-auto text-center "> Manage - <b>
                        <?php echo $title; ?>
                    </b></h1>
                <div class="m-auto Poppins h-full gap-4 mt-40 w-full text-md flex flex-col items-center justify-center m-auto"
                    autocomplete="off" action="" method="post">
                    <?php if (isset($_GET['Game'])) { ?><b class="flex items-center ">Quiz pin - <i
                                class="animate-pulse text-2xl text-orange-500">
                                <?php echo $gpin; ?>
                            </i></b><br>
                    <?php } ?>
                    ...
                    <input type="button" onclick="ranks(<?php echo $gpin; ?>)"
                        class="px-4 py-2 text-green-500 rounded hover:bg-green-500 hover:text-white border-2 border-green-500"
                        value="Open Submissions Page">
                    <input type="button" onclick="launch(<?php echo $gpin; ?>)" name="action"
                        class="px-4 py-2 text-blue-600 rounded hover:bg-blue-600 hover:text-white border-2 border-blue-600"
                        value="Launch" />
                    <input type="button" onclick="deleteQuiz(<?php echo $gpin; ?>)"
                        class="px-4 py-2 text-red-500 rounded hover:bg-red-500 hover:text-white border-2 border-red-500"
            ...
            if ($_GET['action'] == "Ranks") { ?>
                ...
                <?php
                $sql = "SELECT *,(submitTime-loginTime)as Time FROM $tbl_usr WHERE gamepin='$gpin' ORDER BY marks DESC,Time;";
                $query = mysqli_query($con, $sql);
                $i = 1;
                ...
                if ($entries > 0) {
                    ...
                    <h1 class="text-orange-500 text-3xl text-center Times">Submitted By</h1><br>

                    <!-- <div class="flex justify-center "><input type="button"
                    onclick="location.href='checkranks.php?gpin=<?php echo $gpin; ?>';"
                    class="px-4 py-2 text-orange-500 rounded hover:bg-orange-500 hover:text-white border-2 border-orange-500"
                    value="Open Ranking Page">
                </div><br> -->
                    <table class="text-center flex justify-evenly w-screen m-auto">
                        <!-- <div class="text-center flex items-center justify-evenly"></div> -->
                        <tr class="text-lg Gotu m-auto">
                            <td class="p-2 m-1 font-bold bg-orange-300 border-2 border-orange-500 rounded">Rank</td>
                            <td class="p-2 m-1 font-bold bg-orange-300 border-2 border-orange-500 rounded">User ID</td>
                            <td class="p-2 m-1 font-bold bg-orange-300 border-2 border-orange-500 rounded">Name</td>
                            <td class="p-2 m-1 font-bold bg-orange-300 border-2 border-orange-500 rounded">Score</td>
                            <td class="p-2 m-1 font-bold bg-orange-300 border-2 border-orange-500 rounded">Time Taken</td>
                        </tr>
                        <?php
                        while ($val = mysqli_fetch_assoc($query)) {
                            if ($val['marks'] != '') {
                                ?>
                                <tr class="text-center odd:bg-orange-100">
                                    <td class="p-2 m-1 font-semibold  border border-orange-500 rounded">
                                        <?php echo $i; ?>
                                    </td>
                                    <td class="p-2 m-1 font-semibold bg-temp border border-orange-500 rounded">
                                    ...
                                    <td class="p-2 m-1 font-semibold bg-temp border border-orange-500 rounded">
                                        <?php echo number_format($val["Time"], 2); ?> Sec
                                    </td>
                                </tr>
            ...
            <form action="" class="m-auto flex items-center justify-center mt-4"><input type="submit"
                    class="bg-transparent hover:bg-orange-500 text-orange-500 Gotu hover:text-white py-1 px-2 border border-orange-500 hover:border-transparent rounded m-auto"
                    value="Save to File" name="saveInTxt">
                <input name="gpin" type="hidden" value="<?php echo $gpin ?>">
            </form>
            </div>
        ...
        <?php if ($_GET['action'] == "Launch") {
            ...
            $con->query($q5);
            $q6 = "UPDATE login SET gamepin='$gpin';";
            $con->query($q6);
            ?>
            <title>Launching |
                <?php echo $title; ?>
                        ...
                        <div class="mt-4">
                            <div class="flex">
                                <input type="text" required name="name" placeholder="Enter Name"
                                    class="w-full px-4 py-2 mt-2 focus:border-0 border focus:border-0 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600"
                                    required>
                            </div>
                            <div class="mt-4">
                            ...
                            <select id="class" name="classNumber"
                                class="m-auto flex items-center justify-center w-full focus:outline-none w-40 border-blue-600 border text-center rounded"
                                required />
